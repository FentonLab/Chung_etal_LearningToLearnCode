...
set(gca,'FontSize',10)
axis tight %necessary to add for time_ticks and z_ticks resampling Dino
...
time_ticks(time_ticks==0) = 1;%fix zeros Dino
for i=1:length(time_ticks)
  new_ticks{i} = num2str(round(time(time_ticks(i)),0)); % fix decimal Ain
end;

z_ticks = get(gca,'YTick');
z_ticks(z_ticks==0) = 1;%fix zeros Dino
