...
Animal_name = {'PP3','PP4','PP7','PP15','PP23'};  %Initial training
...
%-[0] Load the data
raw_slope_PP3=load('CSDSlope_041217_pp3_conflict.mat');  
raw_slope_PP4=load('CSDSlope_041217_PP4_conflict.mat');  
raw_slope_PP7=load('CSDSlope_100918_PP7_conflict.mat');     
raw_slope_PP15=load('CSDSlope_100918_PP15_conflict.mat');  
raw_slope_PP23=load('CSDSlope_100918_PP23_conflict.mat');  



%- Truncate PP10 PP14 to 10 sessions as in PP17--------------------------------
Total_number_session =8; % Total number to truncate
...
slope_PP3=raw_slope_PP3.Slope_data(3:10,:,:,:);
slope_PP4=raw_slope_PP4.Slope_data(3:10,:,:,:);
slope_PP7=raw_slope_PP7.Slope_data(Session_selection,:,:,:);
slope_PP15=raw_slope_PP15.Slope_data(Session_selection,:,:,:);
slope_PP23=raw_slope_PP23.Slope_data(Session_selection,:,:,:);
...
% flag_equal=1;
% if flag_equal
[Total_number_session, num_Stimulus_different, num_channel, stimulus_experiment_number]=size(slope_PP23);
num_Stimulus_different16=size(slope_PP3,3);   
% else 
...
chid_PP3 = [1, 1, 1, 3, 13];% depth:9 contA: [1, 1, 2, 3, 12] for conflict [3,13] for contB: [1, 3, 2, 2, 12];
chid_PP4 = [1, 4, 6, 6, 14]; % delpth:10 contA [1, 4, 6, 7, 14] for contB: [1, 4, 5, 6, 14];
%chid_PP5 = [12, 17,  19, 20];% do not use for analysis
chid_PP7 = [10, 15, 17, 18, 26]; %depth:24/6
chid_PP15 =[11, 16, 18, 19, 25]; %depth: 24/8
%chid_PP12 =[11, 16, 18, 19, 30]; %depth: 22/8 contB[1,1,18,19,30]
chid_PP23 =[4, 13, 14, 15, 22];

raw_chid=[chid_PP3; chid_PP4; chid_PP7; chid_PP15;chid_PP23];
...
%%
Slope_data(:,1:2:11,1:num_Stimulus_different16,1)=slope_PP3(:,1:6,:);
Slope_data(:,1:2:11,1:num_Stimulus_different16,2)=slope_PP4(:,1:6,:);
Slope_data(:,:,:,3)=slope_PP7(:,1:11,:);
Slope_data(:,:,:,4)=slope_PP15(:,1:11,:);
Slope_data(:,:,:,5)=slope_PP23(:,1:11,:);
...
figure;
    for j=1:Total_num_channel_analysis
        ...
        hold on
        test_error=errorbar(Intensity_list(1:2:11),avg_slope_animal_normalized(i,1:2:11,j),std_slope_animal_normalized(i,1:2:11,j));
